  ...
  <div id="app">
    ...
    <main>
      ...
      <div class="container" v-else>
        <h1 class="results__heading" v-if="searchResults.length">
          {{ searchResults.length }} results matching "{{ searchKeywords }}"
        </h1>
        <h1 class="results__heading" v-else>
          No results matching "{{ searchKeywords }}"
        </h1>
        ...
        <ul class="results__search">
          ...
          >
            <header class="result__header">
              ...
              <span
                tabindex="0"
                v-for="(topic, idx) in result.topics"
                :key="idx"
                class="tag"
        ...
        if (this.searchKeywords.indexOf('topic:') > -1) {
          this.searchResults = this.content.filter((result) => {
            ...
            for (let i = 0; i < topics.length; i++) {
              if (
                topics[i].indexOf(
                  this.searchKeywords.toLowerCase().replace(filter, '')
                ) > -1
              ) {
                topicExists = true
              }
          ...
          this.searchResults = this.content.filter((result) => {
            return (
              (result.text || '')
                .toLowerCase()
                .indexOf(this.searchKeywords.toLowerCase()) > -1 ||
              (result.name || '')
                .toLowerCase()
                .indexOf(this.searchKeywords.toLowerCase()) > -1
            )
